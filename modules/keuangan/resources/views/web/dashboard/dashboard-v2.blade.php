...
<head>
    ...
    <title>Dashboard — {{ config('app.name', 'Finance') }}</title>
    ...
    <style>
        ...
        * { font-family: 'Inter', sans-serif; }
        ...
        .donut-wrap::before {
            ...
            /* expense ratio out of total (income+expense) calculated via PHP in data-* */
            background: conic-gradient(
                #386650 0deg var(--donut-fill, 144deg),
                #DCE8DE var(--donut-fill, 144deg) 180deg
            );
        }
        /* inner cutout */
        .donut-wrap::after {
...
@php
    /* ── Computed helpers ── */
    $totalAll     = ($totalIncome ?? 0) + ($totalExpense ?? 0);
    $expensePct   = $totalAll > 0 ? round(($totalExpense / $totalAll) * 100) : 0;
    $incomePct    = $totalAll > 0 ? round(($totalIncome  / $totalAll) * 100) : 0;
    $donutDeg     = round(($expensePct / 100) * 180);   // 0-180 deg for the semi-circle
...
<div class="flex min-h-screen">
    ...
    {{--   MAIN CONTENT                                  --}}
    {{-- ═══════════════════════════════════════════════ --}}
    <main class="flex-1 px-6 py-6 lg:px-8 lg:py-7 overflow-x-hidden">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
            ...
            {{-- 1. Spent this month → Total Pengeluaran --}}
            <div class="card bg-white rounded-2xl p-5 shadow-sm">
                <div class="flex items-end justify-between gap-2">
                    <div>
                        <p class="text-xs font-medium text-gray-400 mb-1.5">Pengeluaran {{ $periodLabel ?? 'Bulan Ini' }}</p>
                        <p class="text-[1.6rem] font-bold text-gray-900 leading-none">{{ format_rupiah($totalExpense ?? 0) }}</p>
                    </div>
                    {{-- mini bar chart (CSS) --}}
                    <div class="flex items-end gap-[3px] h-11 shrink-0">
                        @foreach([45, 70, 55, 100, 65, 80, 35] as $h)
                            <div class="w-[6px] rounded-full"
                                 style="height:{{ $h }}%; background-color:{{ $h===100 ? '#386650' : ($h>=70 ? '#6a9e78' : '#B9D1BF') }}"></div>
                        @endforeach
                    </div>
                </div>
            </div>
            ...
            <div class="card bg-white rounded-2xl p-5 shadow-sm">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-[1.6rem] font-bold text-gray-900 leading-none">{{ format_rupiah($totalIncome ?? 0) }}</p>
                    </div>
                </div>
            </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-4 mb-4">
            ...
            <div class="lg:col-span-6 card bg-white rounded-2xl p-6 shadow-sm flex flex-col">
                {{-- header row --}}
                <div class="flex items-center justify-between mb-5">
                    ...
                    <div class="relative inline-block">
                        <select onchange="window.location.href='?period='+this.value" class="appearance-none bg-[#F4F3F0] text-[11px] font-medium text-gray-500 pl-3 pr-7 py-1.5 rounded-lg cursor-pointer hover:bg-gray-200/60 transition border-0 focus:ring-0">
                ...
                <div class="flex gap-3 mb-5">
                    <div class="flex-1 bg-[#F4F3F0] rounded-xl px-4 py-3">
                        ...
                        <div class="flex items-baseline gap-1.5">
                            ...
                            @if(isset($totalIncome) && $totalAll > 0)
                                <span class="text-[10px] font-bold text-[#386650]">+{{ $incomePct }}%</span>
                            @endif
                        </div>
                    </div>
                    <div class="flex-1 bg-[#F4F3F0] rounded-xl px-4 py-3">
                        ...
                        <div class="flex items-baseline gap-1.5">
                            ...
                            @if(!$balancePositive)
                                <span class="text-[10px] font-bold text-red-400">-{{ $expensePct }}%</span>
                            @else
                                <span class="text-[10px] font-bold text-[#386650]">+{{ $incomePct }}%</span>
                            @endif
                        </div>
            ...
            <div class="lg:col-span-3 card bg-white rounded-2xl p-6 shadow-sm flex flex-col">
                ...
                <p class="text-[11px] text-gray-400 mb-2">Total Expense</p>
                <p class="text-[1.7rem] font-bold text-[#386650] mb-1 leading-tight">{{ format_rupiah($totalExpense ?? 0) }}</p>
                @php
                    $expenseIncomePctChange = ($totalIncome > 0)
                        ? round((($totalExpense / $totalIncome) - 1) * 100)
                        : 0;
                @endphp
            ...
            <div class="lg:col-span-3 card bg-white rounded-2xl p-6 shadow-sm flex flex-col items-center justify-center text-center">
                {{-- avatar initial --}}
                <div class="w-16 h-16 rounded-full bg-[#DCE8DE] flex items-center justify-center text-[#386650] text-2xl font-extrabold mb-3 ring-4 ring-[#F4F3F0]">
                    {{ strtoupper(substr(Auth::user()->name ?? 'U', 0, 1)) }}
                </div>
                <h3 class="text-[15px] font-bold text-gray-900 mb-0.5">{{ Auth::user()->name ?? '-' }}</h3>
                <p class="text-[11px] text-gray-400 mb-5">{{ Auth::user()->email ?? '-' }}</p>

                {{-- stats row --}}
                <div class="flex items-center gap-5 w-full justify-center">
                    <div class="text-center">
                        <p class="text-[10px] text-gray-400 mb-0.5">Kategori</p>
                        <p class="text-lg font-bold text-gray-900">{{ isset($categories) ? $categories->count() : 0 }}</p>
                    </div>
                    <div class="w-px h-8 bg-gray-100"></div>
                    <div class="text-center">
                        <p class="text-[10px] text-gray-400 mb-0.5">Transaksi</p>
                        <p class="text-lg font-bold text-gray-900">{{ isset($transactions) ? $transactions->total() : 0 }}</p>
                    </div>
                    <div class="w-px h-8 bg-gray-100"></div>
                    <div class="text-center">
                        <p class="text-[10px] text-gray-400 mb-0.5">Saldo</p>
                        <p class="text-lg font-bold {{ $balancePositive ? 'text-[#386650]' : 'text-red-500' }}">
                            {{ $balancePositive ? '+' : '' }}{{ round(($balance / max($totalIncome, 1)) * 100) }}%
                        </p>
                    </div>
                </div>
            </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-4">
            ...
            <div class="lg:col-span-4 card bg-white rounded-2xl p-6 shadow-sm">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <a href="{{ route('dashboard') }}" class="text-[11px] text-[#386650] font-semibold hover:underline">Lihat Semua</a>
                </div>
