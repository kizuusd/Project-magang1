        ...
        <div class="relative bg-white rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:max-w-lg w-full"
            ...
            <div class="p-6">
                <div class="flex items-center justify-between mb-5">
                    <div>
                        <h2 class="text-xl font-bold text-gray-900 leading-tight">Tambah Transaksi</h2>
                        <p class="text-sm text-gray-500">Catat pemasukan atau pengeluaran baru</p>
                    </div>
                ...
                <form method="POST" action="{{ route('transactions.store') }}" id="transaction-create-form">
                    ...
                    <div class="mb-5">
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <label class="relative cursor-pointer">
                                <input type="radio" name="type" value="income" class="peer sr-only" onchange="filterCategoriesCreate()"
                                       {{ old('type', request('type', 'expense')) === 'income' ? 'checked' : '' }}>
                            ...
                            <label class="relative cursor-pointer">
                                <input type="radio" name="type" value="expense" class="peer sr-only" onchange="filterCategoriesCreate()"
                                       {{ old('type', request('type', 'expense')) === 'expense' ? 'checked' : '' }}>
                                <div class="flex items-center justify-center p-3 border-2 border-gray-100 rounded-xl
                    ...
                    {{-- Kategori & Tanggal (2 columns) --}}
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
                        <div>
                            <label for="create_category_id" class="block text-sm font-medium text-gray-700 mb-1.5">Kategori</label>
                        ...
                        <div>
                            <label for="create_transaction_date" class="block text-sm font-medium text-gray-700 mb-1.5">Tanggal <span class="text-red-500">*</span></label>
                    ...
                    {{-- Jumlah --}}
                    <div class="mb-5">
                        <label for="create_amount" class="block text-sm font-medium text-gray-700 mb-1.5">Jumlah <span class="text-red-500">*</span></label>
                        <div class="relative">
                            ...
                            <input type="number" name="amount" id="create_amount" value="{{ old('amount') }}" placeholder="0" min="1"
                    ...
                    {{-- Deskripsi --}}
                    <div class="mb-6">
                        <label for="create_description" class="block text-sm font-medium text-gray-700 mb-1.5">Deskripsi</label>
                        <input type="text" name="description" id="create_description" value="{{ old('description') }}" placeholder="Contoh: Belanja Bulanan"
                               class="block w-full rounded-xl border-gray-200 shadow-sm focus:border-[#386650] focus:ring-[#386650] sm:text-sm bg-[#F4F3F0]">
                        @error('description')
                            <p class="mt-1.5 text-xs text-red-500">{{ $message }}</p>
                        @enderror
...
<script>
    function filterCategoriesCreate() {
        const modal = document.getElementById('transaction-create-form');
        if (!modal) return;
