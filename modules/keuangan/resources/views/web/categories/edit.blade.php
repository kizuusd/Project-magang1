<div x-data="{ open: {{ (old('_method') === 'PUT' && old('category_id') == $category->id && $errors->any()) ? 'true' : 'false' }} }"
     @open-edit-category-modal-{{ $category->id }}.window="open = true"
     x-show="open" 
        ...
        <div class="relative bg-white rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:max-w-lg w-full"
            ...
            <div class="p-6 sm:p-8">
                <div class="flex items-center justify-between mb-5">
                    <div>
                        <h2 class="text-xl font-bold text-gray-900 leading-tight">Edit Kategori</h2>
                        <p class="text-sm text-gray-500">Ubah rincian kategori Anda</p>
                    </div>
                ...
                <form method="POST" action="{{ route('categories.update', $category) }}">
                    ...
                    <div class="mb-6">
                        <label for="edit_category_name_{{ $category->id }}" class="block text-sm font-medium text-gray-700 mb-2">
                            Nama Kategori <span class="text-red-500">*</span>
                        </label>
                        <input type="text" name="name" id="edit_category_name_{{ $category->id }}" value="{{ old('name', $category->name) }}"
